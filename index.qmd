...
# Define the calculate_annotations function
calculate_annotations <- function(mat) {
  data <- as.vector(mat)
  ...
  result <- matrix("", nrow = nrow(mat), ncol = ncol(mat))
  for (i in 1:nrow(mat)) {
    for (j in 1:ncol(mat)) {
      if (mat[i, j] == low || mat[i, j] == high || mat[i, j] == median) {
        result[i, j] <- round(mat[i, j], 2)
      }
    }
  }
  return(result)
...
pheatmap(ica_weights,
         ...
         angle_col = 0,
         display_numbers = calculate_annotations(ica_weights),
         number_color = "black",
         border_color = NA)
...
global badges_covs pct_g1 pct_g2 pct_g4 pct_g5 pct_g6 pct_g7 pct_g8 free_account average_student_badges_pre total_teacher_logins_pre total_associated_students total_associated_classes acct_created_days mutliple_classroom_grades school_start_days school_start_date_missing pre_intv_email2_open pre_intv_email1_open
...
quietly areg average_student_badges ib16.condition $badges_covs [aweight=total_associated_students], absorb(school_fixed_effect)
...
rownames(imod_fast$M) <- variable_names[names(ICA_cols)]
...
imod_fast$M %>%
  ...
  arrange(desc(`IC 1`)) %>%
  mutate(
    across(c(`IC 1`, `IC 2`, `IC 3`), ~ round(., 2))
  ) %>%
  gt() %>%
  fmt_number(
    columns = c(`IC 1`, `IC 2`, `IC 3`),
    decimals = 2
  ) %>%
  tab_style(
    style = cell_borders(
...
marginal_effects <- t(
  ...
  # mutate(
  #   Activity = variable_names[rownames(.)],
  #   .before = 1
  # ) %>%
  arrange(desc(`Effect of 1 SD`))

marginal_effects %>%
  gt(rownames_to_stub = T) %>%
  fmt_number(
    columns = c(`Effect of 1 SD`, `Effect of 1 Unit`),
    decimals = 2
  )
...
ica_results %>%
  ...
  tab_source_note(
    source_note = paste("Variance Accounted For:",
                        paste(round(imod_fast2$vafs * 100, 2), collapse = "%, "),
                        "%")
  ) %>%
  cols_label(
    `IC 1` = "IC 1",
