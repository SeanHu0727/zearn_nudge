...
gt_table <- gt(merged_summaries) %>%
  ...
  tab_spanner(label = "ln(Badges + 1)",
              columns = c(coef_se_fe, coef_se_fe_subsample)) %>%
  rows_add(
    term = c(
      "Average Intercept",
      "R-squared",
      "N (Teachers, Classes, Weeks, Total)"
    ),
    coef_se_fe = c(
      format_value(mean(fixef(ica_fe_model))),
      format_value(summary(ica_fe_model)$r.squared[1]),
      paste(
        format_value(n_distinct(
          index(ica_fe_model$model)$Adult.User.ID
        )),
        format_value(pdim(ica_fe_model)$nT$n),
        format_value(pdim(ica_fe_model)$nT$T),
        format_value(pdim(ica_fe_model)$nT$N),
        sep = ", "
      )
    ),
    coef_se_fe_subsample = c(
      ...
      paste(
        format_value(n_distinct(
          index(ica_fe_subsample$model)$Adult.User.ID
        )),
        format_value(pdim(ica_fe_subsample)$nT$n),
        format_value(pdim(ica_fe_subsample)$nT$T),
        format_value(pdim(ica_fe_subsample)$nT$N),
        sep = ", "
      )
    )
  ) %>%
  fmt_markdown(columns = c(coef_se_fe, coef_se_fe_subsample))
...
habits_fe_model <- lm(Badges.per.Active.User ~ Mon + Tue + Wed + Thu + Fri + Sat +
                        month_1 + month_2 + month_3 + month_4 + month_5 +
                        month_9 + month_10 + month_11 +
                        Total_Minutes_on_Zearn + mean_streak + mean_time_lag +
                        MDR.School.ID, data = df)
habits_fe_model <- lm(Badges.per.Active.User ~ Mon + Tue + Wed + Thu + Fri + Sat +
                     month_1 + month_2 + month_3 + month_4 + month_5 +
                     month_9 + month_10 + month_11 +
                     Total_Minutes_on_Zearn + mean_streak + mean_time_lag +
...
gt_table <- habits_fe_results %>%
  ...
  filter(!grepl("MDR.School.ID", Variable)) %>%
  gt() %>%
  fmt_markdown(columns = everything(), rows = TRUE) %>%
  cols_label(
    Variable = "Variable",
...
In particular, our analysis indicates that shifting 10% of logins from other weekdays to Fridays, without increasing overall platform usage, could boost student lesson completion by around `r round(10 * c(-1/4, -1/4, -1/4, -1/4, 1) %*% habits_fe_model$coefficients[2:6], 2)`%. For the typical teacher, this means switching from one Friday login per month to two while reducing one login from another weekday during that month, resulting in an increase of `r round(14.24 * c(-1/4, -1/4, -1/4, -1/4, 1) %*% habits_fe_model$coefficients[2:6], 2)`% in average weekly lesson completion. This outcome underscores the importance of strategic engagement rather than just login frequency.
...
```{r ICA-no-User-Session}
#| label: tbl-ica-robust
