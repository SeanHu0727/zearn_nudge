...
columns_used <- colnames(teacher_student_usage_subset[c(41:42, 44:46, 49:51)])

# Create the formula
fmla_str <- sprintf("log(Badges.per.Active.User + 1) ~ %s",
                    paste(sprintf("`%s`", columns_used), collapse = " + "))
fmla <- as.formula(fmla_str)

# Define the panel model index
panel_index <- c("Classroom.ID", "Usage.Week", "Teacher.User.ID")

# Create and summarize the 'within' model directly
within_model <- plm(fmla, data = teacher_student_usage_subset,
                    index = panel_index, effect = "twoway", model = "within")
summary(within_model)

# Hausman Test supports Fixed Effects
# re <- plm(fmla, data = teacher_student_usage_subset,
#           index = panel_index, effect = "twoway", model = "random")
