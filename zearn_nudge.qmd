...
teacher_usage <- teacher_usage %>%
  mutate(
    ...
    Event.Type = ifelse(Event.Type == "Resource Downloaded",
                        paste("RD.", Curriculum.Resource.Category),
                        Event.Type)
    )
...
teacher_usage_total <- teacher_usage %>%
  ...
  pivot_wider(names_from = Event.Type, values_from = n, values_fill = list(n = 0))
...
teacher_student_usage_subset <- classroom_student_usage %>%
  inner_join(classroom_teacher_lookup, by = "Classroom.ID") %>%
  mutate(week = lubridate::week(Usage.Week),
         year = lubridate::year(Usage.Week)) %>%
  inner_join(teacher_usage_total, by = c("Teacher.User.ID" = "Adult.User.ID",
                                        "week", "year"))
...
teacher_student_usage_subset <- teacher_student_usage_subset %>%
  mutate(Badges.per.Active.User = ifelse(is.na(Badges.per.Active.User), 0, Badges.per.Active.User)) %>%
...
# Perform ICA and PCA
ica_result <- fastICA(X = teacher_student_usage_subset[14:39],
                      n.comp = 3, row.norm = TRUE)
pca_result <- prcomp(teacher_student_usage_subset[14:39], tol = 0.1)
prop_variance <- summary(pca_result)$importance[2, 1:10]

# Create Elbow Plot with ggplot2
elbow_plot <- ggplot(data.frame(Cluster = 1:10, Variance = prop_variance), aes(x = Cluster, y = Variance)) +
  geom_point() +
  geom_line(color = "blue") +
  labs(x = "N-th Cluster", y = "Proportion of Variance", title = "Elbow Method for Determining Optimal k") +
  theme_minimal()

# Save plot to a file (Uncomment to save the plot)
# ggsave("/Users/sean/Desktop/zearn_nudge/robustness_check/Elbow_plot.png", elbow_plot, dpi = 120)
