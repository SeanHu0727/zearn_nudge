...
Our research used a comprehensive dataset from the Zearn educational platform, covering various aspects of teacher behavior and student performance. This data was collected throughout the 2019-2020 academic year, encompassing several schools across Lousiana. Key variables included teacher logins, file downloads, and specific interactions with educational content. Additionally, we accessed student data at the classroom-week level, including metrics like lesson completion, engagement levels, and instances of learning difficulties. 

The dataset exhibited a rich diversity in both teacher and student engagement metrics. On average, teachers logged into the platform several times per week, with notable variations in the frequency and duration of their interactions. The student performance data indicated a wide range of achievement levels, reflecting the diverse learning environments across different classrooms and schools. Standard deviations for key variables highlighted the variability in educational settings, emphasizing the heterogeneous nature of our data. @tbl-summary displays summary statistics, including median values and interquartile ranges for both teacher and student variables, further highlighting the diversity and complexity of the data.
...
Our research explored the complex connection between teacher conduct and student success, utilizing the extensive data set available from Zearn's educational platform during the 2019-2020 academic year. This intricate combination of teacher behaviors and student performance metrics, carefully compiled with a weekly frequency for each classroom, allowed for a thorough analysis while respecting privacy concerns.
...
```{r}
#| label: fig-scree
#| fig-cap: "Scree Plot for Determining Optimal Number of Components. This plot displays the proportion of variance explained by each independent component (IC). The optimal number of components is indicated by the 'elbow' of the plot, where the variance explained by each additional component is minimal."

elbow_plot

```

The significant finding from the ICA was the prominence of IC1, accounting for `r imod.fast$vafs[1]*100` 14.12% of the variance in teacher Zearn activity, as indicated in @fig-ica-heatmap. This result is significant in nudge engineering, highlighting the need to focus interventions on elements encapsulated by IC1. Conversely, IC2 and IC3, with `r imod.fast$vafs[2]*100` 7.27% and `r imod.fast$vafs[3]*100` 6.56% variance explained, play lesser but still noteworthy roles.
...
variable_names <- c(
  ...
  "RD.foundational_lesson_guidance" = "Foundational Lesson Guidance RD"
)
...
pheatmap(ica_weights,
         ...
         fontsize_col = 12,
         labels_col = c(
           sprintf("IC1 \n %.1f%% of var.", imod.fast$vafs[1] * 100),
           sprintf("IC2 \n %.1f%% of var.", imod.fast$vafs[2] * 100),
           sprintf("IC3 \n %.1f%% of var.", imod.fast$vafs[3] * 100)
           ),
         angle_col = 0,
...
Further examination revealed an empathy-driven process behind this trend. In this context, "empathy" refers to teachers' ability to comprehend and engage with students' challenges, as demonstrated by their focused attention on areas where students struggled or succeeded. The weightings in IC1 for activities like Tower Struggles (`r ica_weights["Tower Struggled",1]`), Fluency Completed (`r ica_weights["Fluency Completed",1]`), Guided Practice Completed (`r ica_weights["Guided Practice Completed",1]`), and Number Gym Activity Completed (`r ica_weights["Number Gym Activity Completed",1]`) were significantly higher compared to other activities. This pattern underlines the empathy-driven process in teaching, where teachers' engagement with the platform is focused on understanding student challenges.

The second component, IC2, showed substantial weightings on variety of Resource Downloads (RD), particularly Small Group Lessons (`r ica_weights["Small Group Lesson RD",2]`) and Whole Group (i.e., the entire class) Word Problems (`r ica_weights["Whole Group Word Problems RD",2]`). We labeled this component as "Groups." IC3, with strong weights on Resource Download (RD) for the Professional Development (PD) course guide (`r ica_weights["PD Course Guide RD",3]`) and course notes (`r ica_weights["PD Course Notes RD",3]`), was labeled "Guidance." This component, accounting for a smaller variance (`r imod.fast$vafs[3]*100`%), was harder to interpret due to its complexity and diverse activities.
...
ica_fd_results <- summary(ica_fd_model,
                      vcov = vcovHC(ica_fd_model,
                                    type = "HC3",
                                    cluster = "group")
                      )$coefficients

# Convert to a gt table
gt_table <- ica_fd_results %>%
  as.data.frame() %>%
...
The correlation we discovered has significant implications. It indicates that an increase in activities associated with IC1, such as encountering one more 'Tower Struggle' than average, can lead to an approximate `r summary(ica_fd_model)$coefficients[-1,"Estimate"] %*% ica_weights["Tower Struggled",1:3] / sd(teacher_student_usage_subset$Tower.Struggled) *100` percent increase in student badges. Although this increase may seem small, it may be substantial in the context of complex human behaviors, where even modest changes often lead to far-reaching effects. This observation highlights the potential of targeted interventions based on our findings to yield significant improvements in educational outcomes.
...
In particular, our analysis indicates that shifting 10% of logins from other weekdays to Fridays, without increasing overall platform usage, could boost student lesson completion by around `r 10 * c(-1/4, -1/4, -1/4, -1/4, 1) %*% habits_fe_model$coefficients[1:5] * 100`%. For the typical teacher, this means switching from one Friday login per month to two while reducing one login from another weekday during that month, resulting in an increase of `r 15.30 * c(-1/4, -1/4, -1/4, -1/4, 1) %*% habits_fe_model$coefficients[1:5] * 100`% in average weekly lesson completion. This outcome underscores the importance of strategic engagement rather than just login frequency.
...
We can't really use fixed effects because there does not exist a sufficient statistic allowing the fixed effects to be conditioned out of the likelihood [@honore1992].
...
t(
  100 * rbind(
    tcrossprod(summary(ica_fd_model)$coefficients[-1,"Estimate"],
               imod.fast$M[,1:3]),
    tcrossprod(summary(ica_fd_model)$coefficients[-1,"Estimate"],
               imod.fast$M[,1:3]) /
